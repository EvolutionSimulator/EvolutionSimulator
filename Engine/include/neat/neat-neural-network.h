 ...
 *
 * @details This structure holds the information about an input connection to a neuron, including the ID of the input neuron and the weight of the connection.
 */
struct NeuronInput {
 ...
 *
 * @details This structure is used to represent a neuron in the neural network constructed from a NEAT genome. It includes the neuron's ID, its bias, and its input connections.
 */
struct FeedForwardNeuron {  // Functional units of the NeuralNetowork, obtained
  ...
  std::vector<NeuronInput> inputs; /*!< A vector of NeuronInput representing the inputs to this neuron. */
};
 ...
 *
 * @details This class translates a Genome into a feed-forward neural network structure.
 */
class NeuralNetwork {
 public:
 ...
 private:
  ...
  std::vector<FeedForwardNeuron> ffneurons_; /*!< Feed-forward neurons used in the network. */
