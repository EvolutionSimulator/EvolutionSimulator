...
void Creature::Bite(Creature* creature)
{
  ...
  {
    max_nutrition = SETTINGS.environment.meat_nutritional_value * creature->GetSize() * 2 * mutable_.GetDiet();
  ...
  else
  {
    ...
    max_nutrition =  SETTINGS.environment.meat_nutritional_value * food_to_eat * 2 * mutable_.GetDiet();
  }
