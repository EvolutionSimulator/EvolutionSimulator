...
Genome predatorGenome(){
  ...
  Link constant_acceleration(start_id, start_id +settings::environment::kInputNeurons, 1);
  genome.AddLink(constant_acceleration);

  Link digest(start_id, start_id + settings::environment::kInputNeurons + 5, 1);
  Link stop_digesting(start_id + 5, start_id +settings::environment::kInputNeurons + 5, - 1);
  genome.AddLink(digest);
  genome.AddLink(stop_digesting);

  Link orient_towards_enemy(start_id + 12, start_id +settings::environment::kInputNeurons + 2, 1/2);
  genome.AddLink(orient_towards_enemy);

  Link orient_towards_meat(start_id + 9, start_id +settings::environment::kInputNeurons + 2, 1);
  ...
  Link activation_bite(id, start_id + settings::environment::kInputNeurons + 4, 1);
  genome.AddLink(distance_bite);
  genome.AddLink(distance_meat);
  genome.AddLink(activation_bite);

  Link slow_rotation(start_id + 4, start_id +settings::environment::kInputNeurons + 2, -0.1);
  genome.AddLink(slow_rotation);
