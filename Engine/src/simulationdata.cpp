...
void SimulationData::UpdateAllCreaturesFromBrain() {
  for (Creature& creature : creatures_) {
    creature.UpdateFromBrain(creature.GetEnvironmentalData());
  }
}
...
/*!
 * If the food density is less than the established in the environment it
 * creates new food entities until that threshold is reached
 */
void SimulationData::GenerateMoreFood() {
  double size = food_entities_.size();
  double max_number = environment_.GetFoodDensity() * environment_.kMapHeight *
                      environment_.kMapWidth;
  while (size < max_number) {
    Food new_food = Food();
    new_food.RandomInitialization(environment_.kMapHeight,
                                  environment_.kMapWidth,
                                  environment_.kMaxCreatureSize);
...
void UpdateGridTemplate(
  ...
  // Remove dead entities from the entities vector and update the grid for alive
  // entities
...
void CheckCollisionsTemplate(
    ...
    std::vector<EntityType2>& entity_vector_2, const Environment& environment) {
        ...
        if (itCell2 != row2.second.end()) {
          ...
          for (auto& entity1_index : entities1) {
            for (auto& entity2_index : entities2) {
              ...
              if (CollisionCircleCircle(
                      tolerance, entity1.GetCoordinates(), entity1.GetSize(),
                      entity2.GetCoordinates(), entity2.GetSize())) {
                // Handle the collision as needed<
                entity_vector_1[entity1_index].OnCollision(
                    entity_vector_2[entity2_index]);
              }
            }
          }
        }
...
void SimulationData::CheckFoodCollisions() {
  CheckCollisionsTemplate<Creature, Food>(
      creature_grid_, food_grid_, creatures_, food_entities_, environment_);
}
